...
##output perturbation with huber loss
...
def parameters_set(Epsilon_set,Lambda_set,Huber_set):
    '''
    set parameters used in this paper
    '''
    global Epsilon_global, Lambda_global, Huber_global
...
def svm_output_train(data, labels):
    '''
    output peturbation
...
def train(data, labels):
    ...
    ''' 
    classifer_output = svm_output_train(data, labels)
...
def run(input_txt_file_url):
    ...
    parameters_set(float(Epsilon_set),float(Lambda_set),float(Huber_set))#input are string, convert to float
